...
function getDocumentFieldAtFieldPath({ document, fieldPath, locale, isFullFieldPath, returnUndefined }) {
    ...
    function getField(docField, fieldPath) {
        ...
        if (!localizedField) {
            return throwOrReturnUndefined(`the value of a field at fieldPath '${getPrefixOf(fieldPath)}' is undefined ` +
        ...
        switch (localizedField.type) {
            ...
            case 'model': {
                if (localizedField.isUnset) {
                    return throwOrReturnUndefined(`the fieldPath '${getPrefixOf(fieldPath, 1)}' of a document '${docId}' of type '${modelName}' points into an object that is not set`);
                }
                if (isFullFieldPath) {
                    if (lodash_1.default.head(fieldPath) !== 'fields') {
                ...
                const fieldName = lodash_1.default.head(fieldPath);
                if (typeof fieldName === 'undefined') {
                    return throwOrReturnUndefined(`the fieldPath '${getPrefixOf(fieldPath, 1)}' of a document '${docId}' of type '${modelName}' must point to a field name`);
                }
                fieldPath = lodash_1.default.tail(fieldPath);
                const childDocField = localizedField.fields[fieldName];
                if (!childDocField) {
                    return throwOrReturnUndefined(`the fieldPath '${getPrefixOf(fieldPath)}' points to a non existing field in a document '${docId}' of type '${modelName}'`);
                }
            ...
            case 'list': {
                ...
                if (typeof itemIndex === 'undefined') {
                    return throwOrReturnUndefined(`fieldPath '${getPrefixOf(fieldPath, 1)}' of a document '${docId}' of type '${modelName}' must point to a list item index`);
                }
                fieldPath = lodash_1.default.tail(fieldPath);
                const listItem = localizedField.items && localizedField.items[itemIndex];
                if (!listItem) {
                    return throwOrReturnUndefined(`the fieldPath '${getPrefixOf(fieldPath)}' points to a non existing list item in a document '${docId}' of type '${modelName}'`);
                }
                return getField(listItem, fieldPath);
            }
            default: {
                const primitiveFieldName = origFieldPath[origFieldPath.length - fieldPath.length - 1];
                return throwOrReturnUndefined(`the fieldPath '${origFieldPathStr}' is illegal, a primitive field '${primitiveFieldName}' of type '${localizedField.type}' cannot be followed by additional field paths`);
            }
        }
    }
    const fieldName = lodash_1.default.head(fieldPath);

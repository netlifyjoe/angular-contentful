...
import { validateUpdateOperationFields } from './document-validations';
...
export function getCreateDocumentThunk({
    ...
    return async ({ updateOperationFields, modelName, contentSourceData, locale }) => {
        ...
        await validateUpdateOperationFields({
            updateOperationFields,
            modelName,
            locale,
            contentSourceData,
...
async function createObjectRecursively({
    ...
    if (objectFieldNames.length > 0) {
        userLogger.warn(
            `unknown fields were provided while creating a document at field path '${fieldPathToString(
                fieldPath
            )}', ignoring unknown fields: '${objectFieldNames.join(', ')}'`
        );
    }
    ...
    if (csiModelField.type === 'object') {
        ...
        if (assetSource) {
            ...
            return {
                field: {
                    type: 'image',
                    value: transformAssetSourceDataForAssetSource({ sourceData, assetSource, logger: userLogger })
                },
                newRefDocumentIds: []
            };
        }
        ...
        if (modelField.source) {
            const assetSource = getAssetSourceBySourceName(assetSources, modelField.source);
            if (assetSource) {
                value = transformAssetSourceDataForAssetSource({ sourceData: value, assetSource, logger: userLogger });
            }
        }
        return {
            field: {
...
export async function convertOperationField({
    ...
    switch (operationField.type) {
        ...
        case 'cross-reference': {
            ...
            if (isOneOfFieldTypes(csiModelField.type, ['json', 'cross-reference'])) {
                return {
                    type: csiModelField.type,
                    value: refObject
                };
